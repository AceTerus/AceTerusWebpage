...
# Headless servers (e.g. the deployed web service) have no display to enumerate;
# fall back to a sane default since the window dimensions are only used when
# actually showing image windows (never on the server, show_image_level=0).
try:
    _monitors = get_monitors()
    _monitor_w, _monitor_h = _monitors[0].width, _monitors[0].height
except (ScreenInfoError, IndexError, Exception):  # noqa: BLE001
    _monitor_w, _monitor_h = 1920, 1080


@dataclass
class ImageMetrics:
    # TODO: Move TEXT_SIZE, etc here and find a better class name
    window_width, window_height = _monitor_w, _monitor_h
